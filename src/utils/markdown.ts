...
export function htmlToMarkdown(html: string, options: MarkdownOptions = {}): string {
  ...
  try {
    ...
    // 2. 处理粗体 (strong, b)
    markdown = markdown.replace(/<strong[^>]*>(.*?)<\/strong>/gi, `${config.strongDelimiter}$1${config.strongDelimiter}`);
    markdown = markdown.replace(/<b[^>]*>(.*?)<\/b>/gi, `${config.strongDelimiter}$1${config.strongDelimiter}`);
    ...
    // 斜体 (em, i)
    markdown = markdown.replace(/<em[^>]*>(.*?)<\/em>/gi, `${config.emDelimiter}$1${config.emDelimiter}`);
    markdown = markdown.replace(/<i[^>]*>(.*?)<\/i>/gi, `${config.emDelimiter}$1${config.emDelimiter}`);
    ...
    // 引用 (blockquote)
    markdown = markdown.replace(/<blockquote[^>]*>(.*?)<\/blockquote>/gis, (match: string, content: string) => {
    ...
    markdown = markdown.replace(indentRegex, (match: string, tagName: string, indentLevel: string, content: string) => {
    ...
    // 处理style缩进
    const styleIndentRegex = /<([^>]+)style="[^"]*(?:text-indent|margin-left):[^"]*"[^>]*>(.*?)<\/\1>/gis;
    markdown = markdown.replace(styleIndentRegex, (match: string, tagName: string, content: string) => {
